...
async function dishAngle(req, res) {
  ...
  if (dish_id == null || state_id == null || sign === undefined || sign === null) {
  ...
  try {
    ...
    const personnelResult = await client.query(
      `SELECT COUNT(*)::int AS count FROM personnel WHERE role = 'Engineer' AND is_working = TRUE`
    );
    if (personnelResult.rows.length === 0) {
      throw new Error('No data of Personnel');
    }
    const { count } = personnelResult.rows[0];
    ...
    res.json({
      success: 'True',
      elevation_angle: dish.elevation_angle,
    });
  } catch (err) {
